...
export const createContactlessBooking = async (req, res) => {
  try {
    ...
    const hydratedBooking = await findHotelBooking(hotelId, createdBooking._id);
    const bookingReference = identity?.bookingReference || createBookingReference(hydratedBooking);
    const mobileSession = await createOrRefreshMobileSession({ hotelId, booking: hydratedBooking, baseUrl: clientBaseUrl });
